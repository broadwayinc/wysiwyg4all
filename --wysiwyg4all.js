...
class Wysiwyg4All {
    ...
    _wrapNode(node, wrapper, appendWhole = false) {
        ...
        if ((stripped || node).textContent && (start || end)) {
            if (start && start === end && startOffset === endOffset)
                range = adjustSelection.bind(this)({
                    node: stripped || node,
        ...
        if (key === "TAB") {
            ...
            if (shift) {
                ...
                if (sweep_array.length) {
                    ...
                    if (hasIndent)
                        adjustSelection.bind(this)({ node: startLine, position: offset });
                }
            } else {
                // indent
                if (sweep_array.length) {
                    ...
                    adjustSelection.bind(this)({ node: tab, position: false });
                }
            }
            return;
        }
    ...
    _observeMutation(track) {
        ...
        //                                 this.insertTabPending_tabString = '';
        //                                 this._adjustSelection.bind(this)({ node: tab, position: false });
        //                             }
    ...
    _append(i, insertAfter, wrap = false, focusElement) {
        ...
        if (insertAfter)
            this.range = adjustSelection.bind(this)({ node: focusElement || insertAfter });
    }
    ...
    async _imageSelected(e) {
        ...
        for (let i = 0; i < feedback.image.length; i++) {
            ...
            if (i === feedback.image.length - 1) {
                this.range = adjustSelection.bind(this)({ node: lasttxt, position: true });
            }
        }
    ...
    command(action) {
        if (!action) return;
        console.log("command", { range: this.range });
        ...
        if (this.styleTagOfCommand[action]) {
            ...
            this.range = adjustSelection.bind(this)({ node: text, position: false });
            } else {
            if (restrictedClass) {
                this.range = adjustSelection.bind(this)({
            ...
            //     // this.range = adjustSelection.bind(this)({ node: text, position: [true, false] });
            //     this.range = adjustSelection.bind(this)({ node: text, position: false });
            // }
            // else {
            //     if (restrictedClass) {
