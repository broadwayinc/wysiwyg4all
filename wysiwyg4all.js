...
class Wysiwyg4All {
  ...
  * @param {boolean} [option.logNormalizeRemoval=false] - When set to true, logs nodes that normalization queues for removal.
   */
  constructor(option = {}) {
    console.log("Wysiwyg4All 1.0.74");
    this.hashtag_regex =
    ...
    this.logNormalizeRemoval = logNormalizeRemoval;
  ...
  _setEventListener(listen) {
    ...
    this._keydown = function (e) {
      // check if there is any potential to remove the this.element
      if (this.logExecution) console.log("_keydown()", { e });
      ...
      if (
        ...
        if (this.logExecution) console.log("_keydown(): dead end");
        e.preventDefault();
        ...
        if (["BACKSPACE", "DELETE"].includes(key)) {
          ...
          if (this.range.collapsed) {
            ...
            if (this.range.startOffset === 0) {
              ...
              if(ceil_prev) {
                if (ceil_prev.getAttribute("contenteditable") === "false") {
                  ceil_prev.remove();
                }
              }
    ...
    this._normalize = function (e) {
      ...
      if (
        ...
        this._isUnSelectableElement(e.target)
      ) {
        return;
      }
        ...
        if (mutation.type === "childList") {
          ...
          if (mutation.removedNodes.length) {
            for (let m of mutation.removedNodes) {
              ...
              if (
                ...
                mutationTarget !== this.element &&
                isMutationTargetEmpty
              ) {
                mutationTarget.remove();
                continue;
              }

              if (mutationTarget === this.element && isMutationTargetEmpty) {
                mutationTarget.append(this._createEmptyParagraph());
                continue;
              }
          ...
          if (mutation.addedNodes.length)
            for (let i of mutation.addedNodes) {
              ...
              if (i.nodeType === 1) {
                if(i.nodeType === 1 && i.classList.contains("_static_")) {
                ...
                if (
                  !(
                    (node.isWysiwygEldestChild || node.isCustomElement) &&
  ...
  _normalizeDocument(normalize) {
    ...
    this._nodeCrawler(
      (n) => {
        if(n.nodeType === 1 && n.classList.contains("_static_")) {
          return n;
        }
        ...
        if (
          n.tagName !== "BR" &&
          n.nodeType === 1 &&
          !n.classList.contains("_media_") &&
          !n.classList.contains("_custom_") &&
          !n.closest("._media_") &&
          !n.closest("._custom_") &&
        ...
        if (
          ...
          !n.closest("._media_") && !n.closest("._custom_") &&
          ...
          if (this.logNormalizeRemoval) {
            console.log("_normalizeDocument(): queued node removal", {
              ...
              parentTagName: n.parentNode?.tagName || null,
            });
          }
          to_remove.push(n);
        }
              ...
              if (f.elementId === c.id && f.source !== c.getAttribute("src")) {
                ...
                while (cIdx--) {
                  if (
                    ...
                    const sourceClip = splitTail.substring(
                      splitTail_length > 0 ? splitTail_length : 0
                    );
